...
const listHandler = async (req) => {
  ...
  if (totalRows > 0) {
    ...
    rows.forEach((item) => {
      item.bid_created_time = moment(item.bid_created_time).format(
        'YYYY-MM-DD HH:mm'
      )
    })
  }
...
const list = async (req) => {
  let rows = []
  ;[rows] = await db.query(
    'select photo,bid_product_number,bid_sum_money from product_chang t1 inner join bidding_chang t2 on t2.product_id=t1.product_id'
  )
  return rows
}
...
const list2 = async (req) => {
  let rows = []
  ;[rows] = await db.query(
    'select avatar,account,bid_product_number,bid_created_time,bid_add_money,bid_sum_money from members t1 inner join product_chang t2 on t2.sid=t1.sid inner join bidding_chang t3 on t3.sid=t1.sid ORDER BY bid_created_time DESC'
  )
...
const list3 = async (req) => {
  ...
  let rows = []
  ;[rows] = await db.query(
    'select * from members t1 inner join product_chang t2 on t2.sid=t1.sid inner join bidding_chang t3 on t3.sid=t1.sid ORDER BY bid_created_time DESC'
  )
  return rows
}
...
router.post('/add', upload.none(), async (req, res) => {
  ...
  if (!data.bid_sum_money) {
    ...
    data.bid_sum_money =
      parseInt(data.bid_add_money) + parseInt(data.bid_sum_money)
  }
